...
try:
    ...
    if idx == -1:
        ...
        print(f"Length difference: {len(after) - len(after_new)}")
...
\textbf{绘图原理与步骤：}\\
\textbf{1. 去除冗余轴名以提升干净度：} 根据最新的图谱美学规范，本系统采用自动化深度文本过滤脚本抹除了自第 16 题板块往后所有新绘制的横轴极右侧向外指引的附加量名（脱壳除 $x$ 极轴标识符）。从底层彻底终结横向多余文字对页面边际的重压干涉，重塑了绝对规整无杂念的不等式宽幅物理空域屏。\\
\textbf{2. 非整数刻度游离标定法：} 针对第 29 题中暴露的严苛分数集 $\frac{7}{10}$ 和 $\frac{11}{2}$，强制禁用了打散或畸变破坏基准整数序列的低级加塞手法，而是通过悬引出显式外挂的无碰撞游离下放轨（加装 \texttt{yshift=-0.35cm} 指令，将非常规的分数域强行在垂直视域中向副车道紧急安全避险防沉），完美躲过了与原偶数位刻度轴字母间的互相遮盖，实现多维数据的安全互不干涉并行落位。
}
\end{document}
"""
