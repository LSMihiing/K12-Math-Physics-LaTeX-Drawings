...
% 原始文件：3.tex，行 674-692
% 类型：tikz
% ---
    \begin{tikzpicture}[>=latex, xscale=.6]
\draw [->](-2,0)--(11,0)node[right]{$\omega t$};
\draw [->](0,-1.5)--(0,1.5)node[right]{$i,u$};
\draw [ultra thick] plot[domain=0:3*3.1416, samples=1000, variable=\x] (\x, {sin(\x r)}) node [below]{$u$};
\draw [very thick] plot[domain=0:10, samples=1000, variable=\x] (\x, {.8*sin(\x-3.1416/2 r)}) node [above]{$i$};
