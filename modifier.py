...
try:
    ...
    # 1. Strip x labels after problem 16
    idx = content.find("解答：不等式与不等式组的解集数轴表示")
    if idx != -1:
        before = content[:idx]
        after = content[idx:]
        after = re.sub(r' node\[below, font=\\small\] \{\$x\$\}', '', after)
        content = before + after
        print("Replaced x labels successfully.")

    # 2. Append Problems 29 and 30
    append_text = r"""
%% ==============================
...
\end{document}
"""
    if "补充题）：含分数不等式" not in content:
        # replace the last \end{document} with append_text
        pos = content.rfind(r"\end{document}")
        if pos != -1:
            content = content[:pos] + append_text
            print("Appended Problems 29 and 30.")
        else:
            print("Error: Could not find \end{document}")
    else:
        print("Problems already appended.")
