...
5. **注册主文件**：在 `main.typ` 中对应绘图类型分类下：
   - 添加 `#import` 语句（别名使用 `分类前缀_描述` 格式）
   - 添加 `=== 描述性标题`（三级标题，使 PDF 目录可跳转到每个绘图）
   - 调用 `#模块名.render()`
   - 绘图之间使用 `#sep()` 分隔
...
- 库文件：`lib/styles.typ` (包含样式函数), `lib/grid-utils.typ` (包含方格绘制), `lib/geo-utils.typ` (包含几何绘制)
- 绘图文件放置于 `figures/` 的子目录下（如 `几何作图/`, `统计图表/`, `线段图/`, `物理/`, `数据表格/`）。
