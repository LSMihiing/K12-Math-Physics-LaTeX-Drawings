...
绘图中经常需要计算坐标。Typst 内置 `calc` 数学模块。以 @案例_尺规取等长线段 为例：
...
#table(
  ...
  [中点], [`let M = ((A.at(0)+B.at(0))/2, (A.at(1)+B.at(1))/2)`],
  [距离], [`let d = calc.sqrt(calc.pow(dx,2) + calc.pow(dy,2))`],
  [角度], [`let angle = calc.atan2(dx, dy)`],
  [极坐标→直角], [`let P = (r * calc.cos(a), r * calc.sin(a))`],
)

完整源代码与绘图原理详见 @案例_尺规取等长线段 。
...
#cetz.canvas(length: 0.8cm, {
  ...
  circle((1, 1), radius: 0.08, fill: black,
         stroke: none)
  content((1, -0.1), text(size: 8pt)[实心点])
  circle((4, 1), radius: 0.6,
         stroke: 0.8pt + black)
  content((4, -0.1), text(size: 8pt)[空心圆])
  circle((7, 1), radius: 0.6, stroke: 0.8pt,
         fill: rgb("#BBDEFB"))
  content((7, -0.1), text(size: 8pt)[填充圆])
})
```)
