...
#let render() = {
    ...
    align(center + bottom)[
      #cetz.canvas(length: 1cm, {
        ...
        let sectors = (
          ...
          (start: 34deg, stop: 124deg, label-angle: 79deg, label-radius: r * 0.6, text: [书法\ 25%]),
          (start: 16deg, stop: 34deg, label-angle: 25deg, label-radius: r * 1.2, text: [足球]),
          (start: -56deg, stop: 16deg, label-angle: -20deg, label-radius: r * 0.6, text: [篮球\ 20%]),
        )
  ...
  block(
    ...
    + *饼图构造*：
      - 按各项目所占比例换算圆心角，使用 `arc` 的 `mode: "PIE"` 直接绘制标准扇形。
      - 标签按扇形中间角定位，小扇形“足球”标签适当外移以避免拥挤。
